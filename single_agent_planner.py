    ...
    for c in constraints:
        if c['agent'] == agent:
            ...
            if timestep not in constraints_table:
                constraints_table[timestep] = [c]
            else:
                constraints_table[timestep].append(c)
...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    # No constraints at timestep
    if next_time not in constraint_table:
        return False

    constraints_for_timestep = constraint_table[next_time]

    for c in constraints_for_timestep:
        if len(c['loc']) == 1:
            # Vertex constraint
            if c['loc'][0] == next_loc:
                return True
